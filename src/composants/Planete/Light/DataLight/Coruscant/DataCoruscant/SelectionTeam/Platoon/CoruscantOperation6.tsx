...
const CoruscantOperation6: React.FC = () => {
  const order = [
    'GAS', 'C-3po', 'KAM', 'Sabine', 'YodaHermit', 'GAS',
    'GMY', 'YodaHermit', 'Holdo', 'Zaalbar', 'GAS',
    'JKR', 'Leia-rebelle', 'Sana', 'Raven-claw'
  ];
      ...
      {[0, 5, 10].map((startIndex, rowIndex) => (
        ...
        >
          {orderedImages.slice(startIndex, startIndex + 5).map((image, index) => (
            <div key={index} className="flex flex-col items-center bg-gray-700 rounded-lg p-4 w-32 flex-shrink-0 snap-center">
              <img
                src={image.src}
                alt={image.alt}
...
export default CoruscantOperation6;